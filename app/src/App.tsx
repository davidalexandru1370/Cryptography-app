...
function App() {
  ...
  useEffect(() => {
    if(columnIndex != 0)
    {
    if(columnIndex == 16 || columnIndex == 32 || columnIndex == 48 || columnIndex == 64)
    {
      resetStyles();
    }
    if(columnIndex === 64)
    ...
    setDD(C);
    setCC(B);
    setBB(unsigned32Add(B, rotateLeft(unsigned32Add(A, E, K[columnIndex], m[g]), S[columnIndex])));
    setAA(D);
  }
}, [columnIndex]);
...
useEffect(() => {
  ...
  // Determine the size of the message in bytes
	...
	let paddingLength: number = size % 64 < 56 ? 56 - (size % 64) : (56 + 64) - (size % 64);
...
useEffect(() => {
  ...
  const formSubmitted = (
    e: any) => {
      e.preventDefault();
      if(e.target.stringGetter.value.length > 0 && e.target.stringGetter.value.length < 56)
  ...
  return (
    ...
    <Table className="horizontalTable">
            <tr style={{borderLeft: 0, borderTop: 0, borderRight: 0, textAlign: "center"}}>
              <td>0</td>
              <td>1</td>
              <td>2</td>
              <td>3</td>
    ...
    <div className="Row">
      <div className="Column">
        <Table className="verticalTable">
            <tr>
              <td style={{borderLeft: 0, borderTop: 0, borderBottom: 0, textAlign: "right"}}>A</td>
              <td style={{background: "paleGreen"}}>0x{A.toString(16)}</td>
            </tr>
            <tr>
              <td style={{borderLeft: 0, borderTop: 0, borderBottom: 0, textAlign: "right"}}>B</td>
              <td style={{background: "yellow"}}>0x{B.toString(16)}</td>
            </tr>
            <tr>
              <td style={{borderLeft: 0, borderTop: 0, borderBottom: 0, textAlign: "right"}}>C</td>
              <td style={{background: "orange"}}>0x{C.toString(16)}</td>
            </tr>
            <tr>
              <td style={{borderLeft: 0, borderTop: 0, borderBottom: 0, textAlign: "right"}}>D</td>
      ...
      <Table className="verticalTable">
            <tr>
              ...
              <td style={{borderRight: 0, borderTop: 0, borderBottom: 0, textAlign: "left"}}>A'</td>
            </tr>
            <tr>
              <td style={{background: "paleGreen"}}>0x{BB.toString(16)}</td>
              <td style={{borderLeft: 0, borderTop: 0, borderBottom: 0, textAlign: "left"}}>B'</td>
            </tr>
            <tr>
              <td style={{background: "yellow"}}>0x{CC.toString(16)}</td>
              <td style={{borderLeft: 0, borderTop: 0, borderBottom: 0, textAlign: "left"}}>C'</td>
            </tr>
            <tr>
              <td style={{background: "orange"}}>0x{DD.toString(16)}</td>
              <td style={{borderLeft: 0, borderTop: 0, borderBottom: 0, textAlign: "left"}}>D'</td>
            </tr>
      </Table>
      </div>
    </div>
    ...
    <div className="finalText">
      <h2>
      After the final computation: 0x{Array.from(result)
